  ...
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-end">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Bienvenido, {session?.user?.name?.split(' ')[0] || 'Usuario'}</h1>
          <p className="text-gray-500 mt-1">Resumen general de tu actividad comercial.</p>
        </div>
        <Link 
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <StatCard title="Cotizaciones" value={quotesCount} icon="description" />
        <StatCard title="Clientes" value={clientsCount} icon="people" />
        <StatCard title="Total Facturable (Neto)" value="$0" icon="payments" />
      </div>

      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <h2 className="text-xl font-bold mb-6">Cotizaciones Recientes</h2>
            ...
            <table className="w-full">
              <thead>
                <tr className="text-left text-xs font-bold text-gray-400 uppercase tracking-wider border-b">
                  <th className="pb-4">Correlativo</th>
              ...
              <tbody className="divide-y">
                {recentQuotes.map((quote) => (
                  <tr key={quote.id} className="text-sm">
                    ...
                    <td className="py-4">{new Date(quote.fechaEmision).toLocaleDateString('es-CL')}</td>
                    <td className="py-4 font-bold">${quote.montoTotal.toLocaleString('es-CL')}</td>
                    <td className="py-4">
