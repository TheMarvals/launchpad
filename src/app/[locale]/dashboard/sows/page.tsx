...
import SowActions from '@/components/SowActions';
...
export default async function SowsListPage({ searchParams, params }: { searchParams: Promise<{ page?: string; sort?: string; dir?: string; estado?: string; q?: string; desde?: string; hasta?: string }>, params: Promise<{locale: string}> }) {
  const {locale} = await params;
  const t = await getTranslations('Sows');
  ...
  const [sows, totalCount, allCount] = await Promise.all([
    timedQuery(prisma.sow.findMany({
      orderBy,
      where,
      include: { client: true },
    ...
    timedQuery(prisma.sow.count({ where }), 'sow.count(filtered)'),
    timedQuery(prisma.sow.count({ where: searchWhere }), 'sow.count(search)'),
  ]);
  ...
  return (
    <div className="space-y-md font-sans">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-xs">
        ...
        <div className="flex flex-row gap-xxs w-full sm:w-auto">            <CsvDownloadButton href={`/api/sows/export?${exportParams.toString()}`} locale={locale} />
            <a 
            href={`/api/sows/template/pdf?locale=${locale}`}
        ...
        {sows.length > 0 ? (
          <>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-canvas border-b border-hairline group">
                    <th className="px-sm py-xs text-caption-uppercase text-muted font-semibold"><SortableHeader label={t('table.number')} field="correlativo" currentSort={sortField} currentDir={sortDir} basePath="/dashboard/sows" /></th>
                    <th className="px-sm py-xs text-caption-uppercase text-muted font-semibold"><SortableHeader label={t('table.client')} field="client" currentSort={sortField} currentDir={sortDir} basePath="/dashboard/sows" /></th>
                    <th className="px-sm py-xs text-caption-uppercase text-muted font-semibold"><SortableHeader label={t('table.issueDate')} field="fechaEmision" currentSort={sortField} currentDir={sortDir} basePath="/dashboard/sows" /></th>
                    <th className="px-sm py-xs text-caption-uppercase text-muted font-semibold"><SortableHeader label={t('table.validity')} field="fechaValidez" currentSort={sortField} currentDir={sortDir} basePath="/dashboard/sows" /></th>
                    <th className="px-sm py-xs text-caption-uppercase text-muted font-semibold"><SortableHeader label={t('table.total')} field="montoTotal" currentSort={sortField} currentDir={sortDir} basePath="/dashboard/sows" /></th>
                    <th className="px-sm py-xs text-caption-uppercase text-muted font-semibold"><SortableHeader label={t('table.status')} field="estado" currentSort={sortField} currentDir={sortDir} basePath="/dashboard/sows" /></th>
                    <th className="px-sm py-xs text-caption-uppercase text-muted font-semibold text-right">{t('table.actions')}</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-hairline">
                  {sows.map((sow) => (
                    <tr key={sow.id} className="hover:bg-canvas/80 transition-colors group">
                      <td className="px-sm py-xs font-medium text-ink">
                        Nº {String(sow.correlativo).padStart(4, '0')}
                      </td>
                      <td className="px-sm py-xs">
                        <div className="font-medium text-ink text-sm">{sow.client.razonSocial}</div>
                        <div className="text-xs text-muted">{sow.client.rut}</div>
                      </td>
                      <td className="px-sm py-xs text-body text-muted">
                        {new Date(sow.fechaEmision).toLocaleDateString(locale)}
                      </td>
                      <td className="px-sm py-xs text-body text-muted">
                        {new Date(sow.fechaValidez).toLocaleDateString(locale)}
                      </td>
                      <td className="px-sm py-xs font-medium text-ink">
                        ${sow.montoTotal.toLocaleString(locale)}
                      </td>
                      <td className="px-sm py-xs">
                        <span className={`inline-flex items-center px-xxs py-[2px] text-caption-uppercase font-semibold border ${
                          sow.estado === 'Aceptada' ? 'border-semantic-success/30 bg-semantic-success/10 text-semantic-success' :
                          sow.estado === 'Borrador' ? 'border-hairline bg-canvas-elevated text-muted' :
            ...
            <div className="sm:hidden divide-y divide-hairline">
              {sows.map((sow, index) => (
                <div key={sow.id} className="animate-fade-in px-sm py-xs space-y-xxs hover:bg-canvas/50 transition-colors" style={{ animationDelay: `${index * 0.1}s` }}>
                  <div className="flex items-start justify-between gap-xxs">
                    <div className="min-w-0 flex-1">
                      <span className="font-medium text-ink text-sm">Nº {String(sow.correlativo).padStart(4, '0')}</span>
                      <p className="text-xs text-muted truncate">{sow.client.rut}</p>
                    </div>
                  ...
                  <p className="text-ink text-sm font-medium">{sow.client.razonSocial}</p>
                  <div className="flex items-center justify-between text-xs text-muted">
                    <span>{new Date(sow.fechaEmision).toLocaleDateString(locale)} → {new Date(sow.fechaValidez).toLocaleDateString(locale)}</span>
                    <span className="font-semibold text-ink">${sow.montoTotal.toLocaleString(locale)}</span>
                  </div>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between p-sm border-t border-hairline">
            ...
            <div className="flex items-center gap-xxs">                  {currentPage > 1 ? (
                <Link
                  href={`/dashboard/sows?page=${currentPage - 1}&sort=${sortField}&dir=${sortDir}`}
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                <Link
                  key={page}
                  href={`/dashboard/sows?page=${page}&sort=${sortField}&dir=${sortDir}`}
